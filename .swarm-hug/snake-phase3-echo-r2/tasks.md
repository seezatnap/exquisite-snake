...
- [x] (#4) Implement delayed ghost-food burst behavior: when food is eaten, queue a cosmetic burst at the corresponding ghost path position exactly 5 seconds later, including handling if the target history sample is unavailable [5 pts] (blocked by #1, #2) (B)
...
- [x] (#6) Add a rewind-ready interface/hook for `EchoGhost` buffer state (snapshot/restore hooks and integration points) so Phase 6 rewind can rewind ghost history without implementing rewind timing in this phase [5 pts] (blocked by #1, #2) (A)
