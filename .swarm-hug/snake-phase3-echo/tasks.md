...
## Follow-up tasks (from sprint review)
- [x] (#12) Cache and reuse the last non-empty ghost trail state so fading ghost rendering remains visible on frames where `snake.update(delta)` does not advance movement (currently `readDelayedTrail(replayState === "fading")` only renders fading on stepped frames, so the trail can disappear between ticks). (blocked by #10) (A)
