...
## Follow-up tasks (from sprint review)
- [C] (#25) Wire the real `MainScene` class into `src/game/config.ts` (`scene: [BootScene, MainScene]`) and remove the `MAIN_SCENE_PLACEHOLDER` so `BootScene` starts the implemented scene logic.
- [C] (#26) Update `tests/game-config-boot.test.mjs` to assert `GAME_CONFIG` registers the `MainScene` class (not a key-only placeholder) to prevent this wiring regression.
