...
- [A] (#2) Implement `src/app/layout.tsx` and `src/app/page.tsx` to provide global Tailwind/font setup and mount a client-side `<Game />` entry point with proper overlay layering for menus/HUD [5 pts] (blocked by #1)
...
- [B] (#20) Fix static-export npm scripts in `package.json` by replacing the broken `start` command (`next start` with `output: "export"`) with a static preview command that serves `out/`.
- [B] (#21) Update `README.md` to document the actual static-export workflow and local preview steps (build/export output in `out/` and how to run it).
