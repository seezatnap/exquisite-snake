...
- [B] (#5) Define `src/game/scenes/MainScene.ts` core scene skeleton plus a Phaser↔React state bridge for game phase, score/high score, and elapsed survival time updates consumed by overlays [5 pts] (blocked by #3)
...
## Follow-up tasks (from sprint review)
- [A] (#23) Replace source-text regex checks in `tests/game-component.test.mjs` with behavioral tests that mount `<Game />`, mock Phaser, and verify single-instance creation plus `destroy(true)` and DOM cleanup on unmount.
- [B] (#24) Add a regression test that ensures `src/styles/globals.css` does not set `body` `background`, `color`, or `font-family`, so layout utility classes keep precedence after the global-style fix.
