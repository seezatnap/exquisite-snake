...
- [A] (#6) Implement collapse-mid-transit handling so any remaining unthreaded segments are teleported instantly to the exit side with correct final ordering/positions [5 pts] (blocked by #5)
...
- [A] (#10) Implement split-snake rendering state during transit so the snake appears at both entry and exit sides while segments are still threading [5 pts] (blocked by #5, #8)
