...
- [A] (#13) Add automated tests and a manual QA matrix covering spawn cadence, traversal, split rendering, collapse edge cases, temporary collision immunity, biome interactions, and ghost/food exclusions [5 pts] (blocked by #9, #10, #12)
...
## Follow-up tasks (from sprint review)
- [A] (#17) Fix `Food.findSafePosition` so portal endpoint exclusions remain strict when blocked endpoints are the only free cells (near-full board); do not fall back to spawning food on blocked endpoint cells, and add a regression test for this edge case.
