...
- [A] (#5) Implement Void Rift gravity well at arena center that nudges movement toward center by 1 tile on a configurable cadence while keeping movement fair and deterministic [5 pts] (blocked by #2)
...
- [B] (#7) Implement biome-specific visual themes (palette/tilemap/background) for Neon City, Ice Cavern, Molten Core, and Void Rift, wired to biome state changes [5 pts] (blocked by #2)
...
- [C] (#14) Remove duplicate biome reset emissions on new run start so `biomeChange` and `biomeVisitStatsChange` fire only once, and add a regression test for single-fire behavior.
