...
- [ ] (#1) Create `src/game/systems/BiomeManager.ts` with biome enum/config, a 45-second timer, deterministic cycle order (Neon City → Ice Cavern → Molten Core → Void Rift → repeat), and clean start/reset behavior per run [5 pts]
